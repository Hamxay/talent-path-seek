...
export function ClassicTemplate({ resume }: { resume: ResumeJSON }) {
  ...
  return (
    <div className="resume-page resume-print-no-shadow bg-white text-slate-900">
      <div className="px-10 py-10">
        <header className="resume-section text-center">
          <h1 className="font-display text-3xl font-extrabold">{p.full_name || "Your Name"}</h1>
          {(contact || links) && (
            <div className="mt-2 text-xs text-slate-700">
              {contact && <div>{contact}</div>}
              {links && <div className="mt-1 break-words">{links}</div>}
            </div>
          )}
        </header>
        ...
        {nonEmpty(resume.summary) && (
          <>
            ...
            <p className="mt-3 text-sm text-slate-700 leading-relaxed">{resume.summary}</p>
          </>
        )}
              ...
              {resume.experience.map((e, idx) => (
                <div key={idx} className="resume-avoid-break">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="shrink-0 text-xs text-slate-600">{joinNonEmpty([e.start_date, e.end_date], " — ")}</div>
                  </div>
                  ...
                  {(e.bullets || []).filter(nonEmpty).length > 0 && (
                    <ul className="mt-2 list-disc pl-5 text-sm text-slate-700 space-y-1">
                      {(e.bullets || []).filter(nonEmpty).slice(0, 10).map((b, i) => (
                        <li key={i}>{b}</li>
                      ))}
                    </ul>
                  )}
